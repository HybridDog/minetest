...
#define STOCHASTIC_TEXTURE_SAMPLING 1
#endif
...
vec4 sample_texture_stochastic(vec2 uv)
{
	// Get triangle info
	float w1, w2, w3;
...
void main(void)
{
	...
	vec4 base = sample_texture_stochastic(world_aligned_uv());
#else
	vec4 base = texture2D(baseTexture, varTexCoord.st).rgba;
#endif
