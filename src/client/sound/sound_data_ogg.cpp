...
/*
 * ISoundDataOpenTODO struct
 */

std::shared_ptr<ISoundDataOpenOgg> ISoundDataOpenOgg::fromOggFile(std::unique_ptr<RAIIOggFile> oggfile,
		const std::string &filename_for_logging)
{
	...
	if (decode_info->length_seconds <= SOUND_DURATION_MAX_SINGLE) {
		return std::make_shared<SoundDataOpenBufferOgg>(std::move(oggfile), *decode_info);
	} else {
		return std::make_shared<SoundDataOpenStreamOgg>(std::move(oggfile), *decode_info);
	}
}

std::shared_ptr<ISoundDataOpenOgg> ISoundDataOpenOgg::fromFile(const std::string &sound_name, const std::string &path)
{
	auto oggfile = std::make_unique<RAIIOggFile>();

	if (ov_fopen(path.c_str(), oggfile->get()) != 0) {
		warningstream << "Audio: Error opening " << path << " for decoding"
				<< std::endl;
		return nullptr;
	}
	oggfile->m_needs_clear = true;

	return fromOggFile(std::move(oggfile), sound_name);
}

std::shared_ptr<ISoundDataOpenOgg> ISoundDataOpenOgg::fromBuffer(const std::string &sound_name, std::string buffer)
{
	auto oggfile = std::make_unique<RAIIOggFile>();

	auto buffer_source = std::make_unique<OggVorbisBufferSource>();
	buffer_source->buf = std::move(buffer);
	...
	return ISoundDataOpenOgg::fromOggFile(std::move(oggfile), sound_name);
}


/*
 * SoundDataOpenBufferTODO struct
 */

SoundDataOpenBufferOgg::SoundDataOpenBufferOgg(std::unique_ptr<RAIIOggFile> oggfile,
		const OggFileDecodeInfo &decode_info) : ISoundDataOpenOgg(decode_info)
{
	m_buffer = oggfile->loadBuffer(m_decode_info, 0, m_decode_info.length_samples);
	if (m_buffer.get() == 0) {
		warningstream << "SoundDataOpenBufferOgg: Failed to load sound \""
...
/*
 * SoundDataOpenStreamOgg struct
 */

SoundDataOpenStreamOgg::SoundDataOpenStreamOgg(std::unique_ptr<RAIIOggFile> oggfile,
		const OggFileDecodeInfo &decode_info) :
	ISoundDataOpenOgg(decode_info), m_oggfile(std::move(oggfile))
{
	// do nothing here. buffers are loaded at getOrLoadBufferAt
}

std::tuple<ALuint, ALuint, ALuint> SoundDataOpenStreamOgg::getOrLoadBufferAt(ALuint offset)
{
...
std::tuple<ALuint, ALuint, ALuint> SoundDataOpenStreamOgg::loadBufferAt(ALuint offset,
